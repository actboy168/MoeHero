ac.game:record(0, '胜利-1')
ac.game:record(1, '萌力-100')
ac.game:record(2, '萌王-1')
ac.game:record(3, '萌杀-1')
...
for i = 1, 12 do
	...
	if p:isPlayer() then
		...
		-- 先认为玩家输了
		p:set_record('萌力-100', p.lose_moe)
		-- 先认为所有人都逃跑
...
ac.game:event '游戏-结束' (function(_, team)
	...
	for i = 1, 12 do
		...
		if hero then
			if hero.yuri then
				p:add_record('百合-1', 1)
				p:set_record('百合控', math.floor(p:get_record '百合-1' / p:get_record '局数-1' * 100))
			end
			if hero.pad then
				p:add_record('平胸-1', 1)
				p:set_record('平胸控', math.floor(p:get_record '平胸-1' / p:get_record '局数-1' * 100))
			end
			if hero.loli then
				p:add_record('萝莉-1', 1)
				p:set_record('萝莉控', math.floor(p:get_record '萝莉-1' / p:get_record '局数-1' * 100))
			end
		end
		p:save_record()
	end
